...
シュミレーションの実行は`simulate`という関数を用います。入力引数の`option`を設定することで、地絡応答や初期値応答、外乱応答などを行うことができます。ここでは地絡を加えたシミュレーションを実行してみましょう。初期値応答や外乱応答などの他のシュミレーションの実行は[シュミレータベースのTutorial](/Tutorials/withSimulation)を参照して下さい。  

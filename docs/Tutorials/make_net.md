...
本シュミレータには、既存の電力システムを定義するための関数が既に2つあり、以下のように実行することで定義できます。  

```matlab
net = network_3bus;  %3busモデルの情報をnetに格納する場合
net = network_68bus; %68busモデルの情報をnetに格納する場合
```

上記の`network_○○bus`という関数は、電力システムを定義する関数と一緒に予め既存の電力システムのパラメータを一つの関数ファイルとして定義しておくことで、関数を実行するだけでパラメータをもとに電力システムを自動で定義してくれるようにしたものになります。各システムの定義については「[Doc](../Docs/power_network.md)」を参照してください。  
